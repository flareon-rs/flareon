...
use sea_query::{Iden, SchemaStatementBuilder, SimpleExpr, Value};
...
pub enum DatabaseError {
    ...
    ForeignKeyNotFound,
    /// Primary key could not be converted from i64 using [`TryFromI64`] trait.
    #[error("Primary key could not be converted from i64")]
    PrimaryKeyFromI64Error,
}
...
impl<T: Into<sea_query::Value>> From<T> for DbValue {
...
impl Database {
    ...
    pub async fn insert<T: Model>(&self, data: &mut T) -> Result<()> {
        ...
        let (identifiers, values): (Vec<_>, Vec<_>) = std::iter::zip(column_identifiers, values)
            .filter(|(identifier, value)| value.is_value())
            .unzip();
        ...
        let insert_statement = sea_query::Query::insert()
            ...
            .values(
                values
                    .into_iter()
                    .map(|value| SimpleExpr::Value(value.unwrap_value()))
                    .collect::<Vec<_>>(),
            )?
    ...
    pub async fn raw_with(
        ...
        let values = values
            ...
            .map(|value| {
                value
                    .to_db_value()
                    .expect_value("raw_with only accepts DbValue::Value parameters")
            })
            .collect::<Vec<_>>();
...
impl TryFromI64 for i64 {
    fn try_from_i64(value: i64) -> Result<Self> {
        Ok(value)
    }
}

impl TryFromI64 for i32 {
    fn try_from_i64(value: i64) -> Result<Self> {
        Ok(value
            .try_into()
            .map_err(|_| DatabaseError::PrimaryKeyFromI64Error)?)
    }
}
...
impl<T: DatabaseField> ToDbValue for Auto<T> {
    fn to_db_value(&self) -> DbValue {
        match self {
            Self::Fixed(value) => value.to_db_value(),
            Self::Auto => DbValue::Auto,
        }
    }
}
