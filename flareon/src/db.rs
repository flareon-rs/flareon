    ...
    pub async fn insert<T: Model>(&self, data: &mut T) -> Result<()> {
        ...
        let mut insert_statement = sea_query::Query::insert()
            ...
            .values(
                filtered_values
                    .into_iter()
                    .map(|value| SimpleExpr::Value(value))
                    .collect::<Vec<_>>(),
            )?
            .or_default_values()
            .to_owned();
...
impl StatementResult {
    ...
    /// Creates a new statement result with the given number of rows affected
    /// and last inserted row ID.
    #[must_use]
    pub(crate) fn new_with_last_inserted_row_id(
        rows_affected: RowsNum,
        last_inserted_row_id: u64,
    ) -> Self {
        Self {
            rows_affected,
            last_inserted_row_id: Some(last_inserted_row_id),
        }
    }
